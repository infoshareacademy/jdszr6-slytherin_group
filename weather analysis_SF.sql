...
select substring(date,1,1) as miesiac
from weather

/*polaczenie roku i miesiaca*/
select substring(date,1,1) || '-' || substring(date,-4,4) as rok_i_miesiac
from weather
...
select max_temperature_f, substring(date,1,1) || '-' || substring(date,-4,4) as rok_i_miesiac,  
avg(max_temperature_f) over 
	(partition by substring(date,1,1) || '-' || substring(date,-4,4) 
	order by substring(date,1,1) || '-' || substring(date,-4,4)) as sr_max_temp_na_miesiac
from weather
...
select min_temperature_f, substring(date,1,1) || '-' || substring(date,-4,4) as rok_i_miesiac,  
avg(min_temperature_f) over 
	(partition by substring(date,1,1) || '-' || substring(date,-4,4) 
	order by substring(date,1,1) || '-' || substring(date,-4,4)) as sr_min_temp_na_miesiac
from weather
where zip_code like '94%';
...
select mean_temperature_f , substring(date,1,1) || '-' || substring(date,-4,4) as rok_i_miesiac,  
avg(mean_temperature_f) over 
	(partition by substring(date,1,1) || '-' || substring(date,-4,4) 
	order by substring(date,1,1) || '-' || substring(date,-4,4)) as sr_mean_temp_na_miesiac
from weather
where zip_code like '94%';
...
select zip_code, substring(date,1,1) || '-' || substring(date,-4,4), avg(mean_temperature_f) over 
	(partition by substring(date,1,1) || '-' || substring(date,-4,4) 
	order by substring(date,1,1) || '-' || substring(date,-4,4)), 
	ntile(4) over 
	(partition by substring(date,1,1) || '-' || substring(date,-4,4) 
	order by substring(date,1,1) || '-' || substring(date,-4,4))
from weather
...
select mean_visibility_miles , substring(date,1,1) || '-' || substring(date,-4,4) as rok_i_miesiac,  
avg(mean_visibility_miles) over 
	(partition by substring(date,1,1) || '-' || substring(date,-4,4) 
	order by substring(date,1,1) || '-' || substring(date,-4,4)) as sr_mean_visibility_na_miesiac
from weather
where zip_code like '94%';
...
select max_visibility_miles _temperature_f , substring(date,1,1) || '-' || substring(date,-4,4) as rok_i_miesiac,  
avg(max_visibility_miles) over 
	(partition by substring(date,1,1) || '-' || substring(date,-4,4) 
	order by substring(date,1,1) || '-' || substring(date,-4,4)) as sr_max_visibility_na_miesiac
from weather
where zip_code like '94%';

select min_visibility_miles , substring(date,1,1) || '-' || substring(date,-4,4) as rok_i_miesiac,  
avg(min_visibility_miles) over 
	(partition by substring(date,1,1) || '-' || substring(date,-4,4) 
	order by substring(date,1,1) || '-' || substring(date,-4,4)) as sr_min_visibility_na_miesiac
from weather
where zip_code like '94%';
...
select count(1), substring(date,1,1) || '-' || substring(date,-4,4) as rok_i_miesiac
from weather w 
WHERE precipitation_inches>0 and zip_code like '94%'
group by substring(date,1,1) || '-' || substring(date,-4,4) 
...
select precipitation_inches, substring(date,1,1) || '-' || substring(date,-4,4) 
from weather w 
where substring(date,1,1) || '-' || substring(date,-4,4) = '1-2014' and precipitation_inches >0 and zip_code like '94%'
order by date; 
...
select precipitation_inches, substring(date,1,1) || '-' || substring(date,-4,4) as rok_i_miesiac,  
max(precipitation_inches) over 
	(partition by substring(date,1,1) || '-' || substring(date,-4,4) 
	order by substring(date,1,1) || '-' || substring(date,-4,4)) as max_opadow
from weather
where zip_code like '94%';
...
avg(precipitation_inches) over 
	(partition by substring(date,1,1) || '-' || substring(date,-4,4) 
	order by substring(date,1,1) || '-' || substring(date,-4,4)) as sr_opadow_na_miesiac
from weather
where zip_code like '94%';
...
select mean_wind_speed_mph , substring(date,1,1) || '-' || substring(date,-4,4) as rok_i_miesiac,  
avg(mean_wind_speed_mph) over 
	(partition by substring(date,1,1) || '-' || substring(date,-4,4) 
	order by substring(date,1,1) || '-' || substring(date,-4,4)) as sr_predkosc_wiatru_na_miesiac
from weather
where zip_code like '94%';
...
select max_wind_Speed_mph, substring(date,1,1) || '-' || substring(date,-4,4) as rok_i_miesiac,  
avg(max_wind_Speed_mph) over 
	(partition by substring(date,1,1) || '-' || substring(date,-4,4) 
	order by substring(date,1,1) || '-' || substring(date,-4,4)) as sr_max_predkosc_wiatru_na_miesiac
from weather
where zip_code like '94%';
...
select max_gust_speed_mph, substring(date,1,1) || '-' || substring(date,-4,4) as rok_i_miesiac,  
avg(max_gust_speed_mph) over 
	(partition by substring(date,1,1) || '-' || substring(date,-4,4) 
	order by substring(date,1,1) || '-' || substring(date,-4,4)) as sr_max_poryw_wiatru_na_miesiac
from weather
where zip_code like '94%';
